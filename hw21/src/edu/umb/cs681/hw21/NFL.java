...
public class NFL {
	...
	public static void main(String args[]) {
		...
		long numberOfTeams = NFLTable.stream().parallel().count();
		System.out.println("---Number of teams in NFL: " + numberOfTeams);

		NFL highestRunRateTeam = NFLTable.stream().parallel().max(Comparator.comparing(NFL::getwinper)).get();
		System.out.println("NFL Team that has highest Win Percentage : " + highestRunRateTeam.getTeam());

		NFL leastPointsTeam = NFLTable.stream().parallel().min(Comparator.comparing(NFL::getPoints)).get();
		System.out.println("Team that has least points: " + leastPointsTeam.getTeam());

		List<NFL> sortedBywinper = NFLTable.stream().parallel().sorted(Comparator.comparing(NFL::getwinper, Comparator.reverseOrder()))
